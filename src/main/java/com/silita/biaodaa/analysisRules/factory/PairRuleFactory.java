        ...
        if(pairRuleList==null) {
            synchronized (lock) {
                if(pairRuleList==null) {
                    ...
                    pairRules.add(new PairRule("(投标企业|中标单位|中标人名称|中标候选人|投标人|中标人|单位名称|候选人|供应商|第1名|第2名|第3名|第一|第二|第三)", "[\\s\\S\\W\\w.]*((部|中心|合作社|队|所|局|站|院|厂|处|苗圃|城|部|店|公司|事务所)[\\s\\S\\W\\w.]*$)"
                            , FD_ONE_NAME));
                    ...
                    pairRules.add(new PairRule("(中标价|((投标).*?(价))|((中标).*?(价格))|报价)", "\\d{1,}\\.\\d{1,}(元|圆|万元|万|w)?", FD_ONE_OFFER));
